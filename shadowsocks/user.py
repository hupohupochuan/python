...
if ssrhead == "1":
    error = True
    while error:
        serverip = safe_input("请输入服务器IP地址:\n")
        error = checkip(serverip)
    error = True
    while error:
        serverport = safe_input("请输入服务器端口:\n")
        error = checkport(serverport)
    error = True
    while error:
        print("请输入本机代理地址，默认127.0.0.1，使用默认请回车")
        localaddress = safe_input()
        if localaddress == "":
    ...
    while error:
        ...
        localport = safe_input()
        if localport == "":
            localport = "1080"
        error = checkport(localport)
    password = safe_input("请输入密码:\n")
...
,"auth_aes128_sha1","auth_aes128_md5","auth_chain_a","auth_chain_b","auth_chain_c","auth_chain_d"]
    ...
    protocolparam = safe_input("请输入协议参数，不使用参数请回车:\n")
